...
var players = (document.getElementsByTagName("th").length)-5; 	// Játékosok száma (THEAD elemek megszámolásával kiszámítva)
...
var szabalyszamlalo = 0;									// A szabályok száma
...
function randomkartya(){
	...
	if (pakli_szamlalo == 0) {
		...
		if (pakli[randomnumber-1][randomchar-1] == 0) {
			...
			/* Kártyahúzás */
			document.getElementById("lapok").innerHTML = "<img src='images/kartya/"+ randomnumber + randomchar +".gif'><br><br>Pakli: "+pakli_szamlalo+" lap";
			...
			if (randomnumber == 13) {
					document.getElementById("szabaly_textbox").innerHTML = "<input type='text' id='ujszabaly'><button onclick='beir(); dobozreset();'>Beír</button>";
					alert("\nKirályt húztál, új szabályt hozhatsz!\n");
			}
		}
	}
}
...
function nullaz(){
	...
	for (var g = 0; g <= szabalyszamlalo; g++) {
		torles(g);
	}
}
...
function pluszpakli(){
	...
	pakli_szamlalo = 52; // Feltölti a pakszabalyszamlaloit
	/* Reseteli a mátrixot */
	for  (var k = 0; k <= 3; k++) {
		for (var j = 0; j <= 12; j++) {
			pakli[j][k] = 1;
...
function beir(){
	szabaly[szabalyszamlalo] = document.getElementById("ujszabaly").value;
	document.getElementById("szabalyok").innerHTML += "<tr><td id='"+ szabalyszamlalo +"'>- " + szabaly[szabalyszamlalo] + " - <img src='images/x-red.gif' onclick='torles(" + szabalyszamlalo + ");'></td></tr>";
	szabalyszamlalo++;
}
function torles(kod){
	var torlesre = document.getElementById("" + kod + "");
	torlesre.parentNode.removeChild(torlesre);
}
function dobozreset(){
	document.getElementById("szabaly_textbox").innerHTML = "Ha királyt húzol, új szabályt írhatsz be";
}